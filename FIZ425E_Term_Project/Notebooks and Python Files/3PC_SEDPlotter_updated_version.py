...
class PLEC4(object):
    ...
    def plot(self):
        ...
        dom = np.logspace(np.log10(0.100e3), np.log10(15.000e3), 3)
...
def plot_sed(db, isrc):
    ...
    writeSEDascii.write("# e_ref")
    dom = np.logspace(np.log10(0.100e3), np.log10(15.000e3), 3)
    for i, value in enumerate(dom):
            writeSEDascii.write(" %.3f \n" % (dom[i] / 1000.0))
    else:
       	    True
    writeSEDascii.close()
...
# Yeni bir ECSV yazma fonksiyonu ekliyoruz
def write_ecsv(jname, model, outdir="3PC_SEDs"):
    """
    Write SED data into ECSV format for the specified pulsar and model.
    """


    if not os.path.exists(outdir):
        os.makedirs(outdir)  # Dizini oluştur


    mev_in_erg = 624151
    e_ref = np.logspace(np.log10(0.100e3), np.log10(15.000e3), 3)
    e2dnde = model.sed(e_ref)/mev_in_erg
    e2dnde_err = model.unc_sed(e_ref)/mev_in_erg
    ...
    header = [
        ...
        "# ---",
        "# datatype:",
        "# - {name: e_ref, unit: GeV, datatype: float64, format: '%.2f'}",
        "# - {name: e2dnde, unit: erg / (cm2 s), datatype: float64, format: '%8.4g'}",
        "# - {name: e2dnde_err, unit: erg / (cm2 s), datatype: float64, format: '%8.4g'}",
        "# meta: !!omap",
        "# - {SED_TYPE: e2dnde}",
        "# schema: astropy-2.0",
    ]

    output_path = os.path.join(outdir, f"{jname}.ecsv")
    with open(output_path, "w", newline="") as f:
        f.writelines([line + "\n" for line in header])
        writer = csv.writer(f, delimiter=" ")
        writer.writerow(["e_ref", "e2dnde", "e2dnde_err"])
        for i in range(len(e_ref)):
            # Burada 3 ondalıklı basamağa yuvarlama işlemi yapıyoruz
            f.write(f"{e_ref[i] / 1e3:.3f} {e2dnde[i]:.3e} {e2dnde_err[i]:.3e}\n")
    


import matplotlib.pyplot as plt

# Plot ve ECSV yazma fonksiyonu
def plot_sed(db, isrc, args):
    """
    Parameters
    ----------
    db : the output of "build_database"
    isrc : integer index of source within the list
    args : command line arguments (for output directory)
    """
    (
        jnames,
        fgl_names,
    ...
    if plecs_bfr[isrc] is not None:
        model = plecs_bfr[isrc]
    elif plecs_b23[isrc] is not None:
        model = plecs_b23[isrc]
    else:
        print(f"Model bulunamadı: {jnames[isrc]}")
        return  # Model olmadığı için işlemi sonlandırıyoruz

    # ECSV formatında çıktı yazma
    write_ecsv(jnames[isrc], model, outdir=args.outdir)  # ECSV formatında yaz

    # SED Plot çizme
    e_ref = np.logspace(np.log10(0.100e3), np.log10(15.000e3), 3)
    e2dnde = model.sed(e_ref) / 624151  # Modelden alınan değer
    e2dnde_err = model.unc_sed(e_ref) / 624151  # Hata değerleri

    # Plot çizme
    plt.figure(figsize=(6, 4))
    plt.errorbar(e_ref / 1e3, e2dnde, yerr=e2dnde_err, fmt="o", label=f"{jnames[isrc]} SED", capsize=5)
    plt.xscale('log')
    plt.yscale('log')
    plt.xlabel("Energy (GeV)")
    plt.ylabel("Flux (erg / cm² s)")
    plt.title(f"SED Plot for {jnames[isrc]}")
    plt.legend()

    # Plot'u kaydetme
    plot_output_path = os.path.join(args.outdir, f"{jnames[isrc]}_sed_plot.png")
    plt.savefig(plot_output_path)
    plt.close()  # Plot'u kapatıyoruz

    print(f"Plot saved: {plot_output_path}")



# Ana fonksiyon (tüm pulsarları işlemek için döngü ekliyoruz)
def main():
    parser = argparse.ArgumentParser(description="Plot one, some, or all pulsar SEDs.")
    parser.add_argument(
        "--outdir",
        type=str,
        default="3PC_SEDs",  # Çıktı dosyasının kaydedileceği dizin
        help="Override the default output directory (./3PC_SEDs).",
    )
    args = parser.parse_args()  # args parametresi burada oluşturuluyor

    # Veriyi ve çıktı dizinini ayarlıyoruz
    db = build_database("3PC_Catalog+SEDs_20230803.fits")  # Gerçek FITS dosyasını buraya girin

    # Pulsarları işlemek için döngü
    for idx in range(len(db[0])):  # db[0] jnames listesi
        plot_sed(db, idx, args)  # args parametresi burada geçiliyor
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="3PC2",
        help="Override the default output directory (./3PC2).",
    )
    args, u = parser.parse_known_args()
    ...
    print(args.catalog)
    db = build_database("3PC_Catalog+SEDs_20230803.fits")  # Dosya adı ile birlikte yolun doğru olduğundan emin olun
